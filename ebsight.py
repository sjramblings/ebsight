...
def create_volume_graph(volume_data):
    ...
    # IOPS comparison
    if (
        volume_data.get("avg_read_ops_p99", 0) > 0
        or volume_data.get("avg_write_ops_p99", 0) > 0
    ):
        print(f"\n   📈 IOPS Breakdown")
        print("   " + "=" * 62)
        ...
        if max_iops > 0:
            # P99 IOPS bars
            read_bar = create_bar(
                volume_data.get("avg_read_ops_p99", 0), max_iops, width
            )
            print(
                f"   P99 Read     {read_bar} {volume_data.get('avg_read_ops_p99', 0):.1f}"
            )

            write_bar = create_bar(
                volume_data.get("avg_write_ops_p99", 0), max_iops, width
            )
            print(
                f"   P99 Write    {write_bar} {volume_data.get('avg_write_ops_p99', 0):.1f}"
            )

            # Peak IOPS bars
            read_peak_bar = create_bar(
                volume_data.get("avg_read_ops_peak", 0), max_iops, width
            )
            print(
                f"   Peak Read    {read_peak_bar} {volume_data.get('avg_read_ops_peak', 0):.1f}"
            )

            write_peak_bar = create_bar(
                volume_data.get("avg_write_ops_peak", 0), max_iops, width
            )
            print(
                f"   Peak Write   {write_peak_bar} {volume_data.get('avg_write_ops_peak', 0):.1f}"
            )
...
def get_volume_metrics(ec2_client, cloudwatch, volume_id):
    ...
    try:
        ...
        for metric_name, metric_params in metrics.items():
            ...
            if response["Datapoints"]:
                if metric_name in ["ReadOps", "WriteOps"]:
                    ...
                    results[f"{metric_name}_p99"] = (
                        max(
                            point["ExtendedStatistics"]["p99"]
                            for point in response["Datapoints"]
                        )
                        / period
                    )
                    results[f"{metric_name}_peak"] = (
                        max(
                            point["ExtendedStatistics"]["p99.9"]
                            for point in response["Datapoints"]
                        )
                        / period
                    )
                    ...
                    results[f"{metric_name}_p99"] = max(
                        point["ExtendedStatistics"]["p99"]
                        for point in response["Datapoints"]
                    )
            else:
        ...
        return {
            ...
            "ReadOps_peak": 0,
            "WriteOps_peak": 0,
            "ReadThroughput_p99": 0,
            "WriteThroughput_p99": 0,
            "ReadThroughput_peak": 0,
            "WriteThroughput_peak": 0,
            "QueueLength_p99": 0,
        }
...
def print_consolidated_summary(volumes_data, show_fsx_recommendations=False):
    ...
    print("   " + "=" * 180)
    print(
        f"   {'Volume ID':<15} {'Mount':<8} {'Size':<12} {'Used %':<8} "
    ...
    for vol in volumes_data:
        ...
        usage_pct = (
            (vol["total_snapshot_size"] / vol["volume_size"] * 100)
            if vol["volume_size"] > 0
            else 0
        )
        ...
        p99_mbps = (
            f"{vol['ReadThroughput_p99']:>7.1f}/{vol['WriteThroughput_p99']:<7.1f}"
        )
        peak_mbps = (
            f"{vol['ReadThroughput_peak']:>7.1f}/{vol['WriteThroughput_peak']:<7.1f}"
        )
    ...
    print(
        f"   {'TOTALS:':<23} {total_size:<12.1f} {'':8} "
        f"{total_p99_iops:^25} "
    ...
    if show_fsx_recommendations:
        ...
        daily_change_percent = (
            (total_daily_change / total_size * 100) if total_size > 0 else 0
        )
        ...
        print(
            f"   Recommended SSD Size (GB):      {total_daily_change:.0f}  ({daily_change_percent:.1f}% of total)"
        )
        print(
            f"   Sustained IOPS Required:        {(total_read_iops_p99 + total_write_iops_p99):.0f}"
        )
        print(
            f"   Peak IOPS Required:             {(total_read_iops_peak + total_write_iops_peak):.0f}"
        )
        print(
            f"   Sustained Throughput (MBps):    {(total_read_mbps_p99 + total_write_mbps_p99):.1f}"
        )
        print(
            f"   Peak Throughput (MBps):         {(total_read_mbps_peak + total_write_mbps_peak):.1f}"
        )
        print("   " + "-" * 80)
...
def analyze_volume_snapshots(
    ec2, cloudwatch, volume_id, volume_name, verbose=False, show_graphs=False
):
    ...
    avg_daily_change_percent = (
        (avg_daily_change / volume_size * 100) if volume_size > 0 else 0
    )
    usage_percentage = (
        (total_snapshot_size / volume_size * 100) if volume_size > 0 else 0
    )
    ...
    if verbose:
        ...
        print(
            f"      Average Daily Change: {avg_daily_change:.2f} GiB/day ({avg_daily_change_percent:.1f}%)"
        )
        ...
        print(
            f"   {'Daily Change Rate':<25} {f'{avg_daily_change:.2f} GiB ({avg_daily_change_percent:.1f}%)':<30}"
        )
...
def main():
 ...
 📊 Intelligent EBS Volume Analyzer
    ...
    parser.add_argument(
        "--csv", "-c", action="store_true", help="Export results to CSV"
    )
    ...
    parser.add_argument(
        ...
        "-f",
        action="store_true",
        help="Show FSx for NetApp ONTAP sizing recommendations",
    )
    ...
    try:
        ...
        try:
            ...
            print(
                f"Name: {next((tag['Value'] for tag in instance.get('Tags', []) if tag['Key'] == 'Name'), 'No Name')}"
            )
            print("=" * 80)
            ...
            for volume in volumes:
                ...
                volume_data = analyze_volume_snapshots(
                    ec2, cloudwatch, volume_id, device_name, args.verbose, args.graph
                )
                if volume_data:
